...
open class SyncOnly : CotejiTask() {
    @Input
    @Option(
        option = "query",
        description = "query to filter the tests in the Source"
    )
    var query: String = ""
    ...
    fun syncOnly() {
        ...
        runCotejiCommand("sync-only --id-update-mode=$idUpdateMode$forceOption \"$query\"")
    }
